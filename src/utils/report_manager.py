...
class AllureManager:
        ...
        for cmd in commands_to_try:
            try:
                ...
                if result.returncode == 0:
                    logger.info(f"Allure工具检测成功: {cmd}, 版本: {result.stdout.strip()}")
                    return True
    ...
    def generate_report(self, open_browser: bool = False) -> bool:
        """生成Allure报告"""
        # 检查是否安装了Allure命令行工具
        if not self._check_allure_available():
            logger.error("未找到Allure命令行工具，请先安装Allure")
            return False
        
        try:
            # 生成报告 - 使用shell=True来确保命令正确执行
    ...
    def serve_report(self, port: int = 8080, use_allure_serve: bool = True) -> bool:
        """启动Allure服务器"""
        if use_allure_serve and self._check_allure_available():
            try:
                # 使用Allure内置serve命令
                ...
                if process.poll() is None:
                    logger.info(f"Allure服务器已启动，访问地址: http://localhost:{port}")
                    return True
                else:
                    logger.error("Allure服务器启动失败")
                    return False
            ...
            # 降级到简单HTTP服务器
            return self._start_simple_http_server(port)
    
    def _start_simple_http_server(self, port: int = 8080) -> bool:
        """启动简单HTTP服务器"""
        try:
            import threading
            import http.server
            ...
            # 检查报告是否存在
            if not (self.report_dir / 'index.html').exists():
                logger.error("报告文件不存在，请先生成报告")
                return False
            ...
            logger.info(f"启动简单HTTP服务器: {server_url}")
            ...
            logger.info(f"已在浏览器中打开报告: {server_url}")
            logger.info("请手动停止程序来关闭服务器")
            ...
            logger.error(f"启动简单HTTP服务器失败: {e}")
            return False
    
    def open_report(self, port: int = 8000):
        """打开生成的报告"""
        ...
        try:
            ...
            # 启动HTTP服务器
            with socketserver.TCPServer(("", free_port), ReportHandler) as httpd:
                server_url = f"http://localhost:{free_port}"
                logger.info(f"启动Allure报告服务器: {server_url}")
            ...
            logger.warning(f"使用file协议打开报告（可能无法正常显示）: {index_file}")
            return False
...
def generate_reports(open_browser: bool = False, include_html: bool = True, use_http_server: bool = True):
    """生成所有报告的便捷函数"""
    # 生成Allure报告
    allure_success = allure_manager.generate_report(open_browser=False)  # 先不打开，由后面控制
    ...
    # 打开报告
    if open_browser and allure_success:
        if use_http_server:
            # 优先使用Allure serve命令
            if not allure_manager.serve_report(use_allure_serve=True):
                # 如果Allure serve失败，使用简单HTTP服务器
                allure_manager.open_report()
        else:
            # 使用旧的file协议方式（不推荐）
            import webbrowser
            index_file = allure_manager.report_dir / 'index.html'
            if index_file.exists():
                webbrowser.open(f'file://{index_file.absolute()}')
                logger.warning("使用file协议打开报告，可能无法正常显示")
